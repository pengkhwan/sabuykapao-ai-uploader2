// apps/worker/app/api/ai/get-gallery-alts/route.ts
...
async function handle(req: NextRequest) {
  try {
    ...
    const items: Array<{ key: string; alt: string }> =
      (doc?.aiPreview?.result?.gallery || [])
        .map((g: any) => ({ key: g?.key, alt: g?.alt }))
        .filter((x: any) => x?.key && x?.alt);

    return withCorsJSON({ ok: true, count: items.length, items });
  } catch (err: any) {
    return withCorsJSON(
